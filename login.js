(function(){
  ...
  async function isAllowed(email){
    // 1) tenta RPC (recomendado)
    try{
      const { data, error } = await sb.rpc('is_email_allowed', { p_email: email });
      if (!error) return !!data;
    }catch{}

    // 2) fallback para tabela allowed_emails (se existir e policy permitir)
    try{
      const { data, error } = await sb.from("allowed_emails").select("email").eq("email", email).maybeSingle();
      if (!error) return !!data;
    }catch{}
